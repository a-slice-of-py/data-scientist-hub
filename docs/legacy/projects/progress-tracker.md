# Progress tracker

## 2022

### 05 / May

#### ![50%](https://progress-bar.dev/50) ⚗️ Time series analysis and forecasting

- [resources](/data-scientist-hub/resources/data-science/time-series)

#### ![50%](https://progress-bar.dev/50) 🐍 Greykite

- [resources](/data-scientist-hub/resources/data-science/time-series#greykite)

### 01 / January

#### ![50%](https://progress-bar.dev/50) 🛠️ Git pre-commit hooks

- [resources](/data-scientist-hub/resources/tools/git-versioning)

#### ![50%](https://progress-bar.dev/50) 🐍 Python packaging best practices

- [Switch to minimal `setup.py`](https://www.scivision.dev/minimal-setup-py-with-prerequisites/)
- [What the heck is `pyproject.toml`?](https://snarky.ca/what-the-heck-is-pyproject-toml/)
- [Transition to `pyproject.toml`](http://ivory.idyll.org/blog/2021-transition-to-pyproject.toml-example.html)
- [Setuptools quickstart](https://setuptools.pypa.io/en/latest/userguide/quickstart.html)

#### ![60%](https://progress-bar.dev/60) 📊 Apache Echarts

- [API Reference](https://echarts.apache.org/en/option.html#title)
- [Examples](https://echarts.apache.org/examples/en/index.html)
- [streamlit-echarts](https://github.com/andfanilo/streamlit-echarts)

## 2021

### 10 / October

#### ![60%](https://progress-bar.dev/60) ⚗️ `Vehicle Routing Problem`

### 04 / April

#### ![60%](https://progress-bar.dev/60) 🐍 `ortools`

- [ortools](https://developers.google.com/optimization/reference/python/index_python)

### 03 / March

#### ![60%](https://progress-bar.dev/60) 🛠️ `reveal.js`

- [reveal.js](https://revealjs.com/)
- [Hosting on GitHub pages](https://martinomensio.medium.com/how-to-host-reveal-js-slides-on-github-pages-and-have-a-tidy-repository-1a363944c38d)

#### ![75%](https://progress-bar.dev/75) 🐍 `pydocstyle`

- [`pydocstyle`](https://github.com/PyCQA/pydocstyle)

#### ![80%](https://progress-bar.dev/80) ⚙️ `TLJH`

- [The Littlest JupyterHub installation](https://tljh.jupyter.org/en/latest/install/custom-server.html)
- [Server spawn failed due to timeout](https://github.com/jupyterhub/the-littlest-jupyterhub/issues/351)
- [Distribute material with nbgitpuller](https://tljh.jupyter.org/en/latest/howto/content/nbgitpuller.html)

### 01 / January

#### ![80%](https://progress-bar.dev/80) 🐍 `OSMnx`

## 2020

### 12 / December

#### ![80%](https://progress-bar.dev/80) 📊 `folium`

- [Intro](https://towardsdatascience.com/how-to-make-stunning-interactive-maps-with-python-and-folium-in-minutes-e3aff3b0ed43)
- [Official docs](https://python-visualization.github.io/folium/)

### 09 / September

#### ![50%](https://progress-bar.dev/50) 🐍 `python-dotenv`

- [Intro](https://github.com/theskumar/python-dotenv)

#### ![50%](https://progress-bar.dev/50) 🐍 Logging

- [Intro to Python logging](https://realpython.com/python-logging/)
- [Stop print and start use `loguru`](https://handyman.dulare.com/stop-using-print-start-using-loguru-in-python/)
- [Loguru](https://github.com/Delgan/loguru)

#### ![10%](https://progress-bar.dev/10) 🐍 `click`

- [Official docs](https://click.palletsprojects.com/en/7.x/)

#### ![60%](https://progress-bar.dev/60) 🐍 `pydantic`

- [Help manual](https://pydantic-docs.helpmanual.io/)
- [How to use pydantic](https://towardsdatascience.com/pydantic-688e897cfd3a)

#### ![75%](https://progress-bar.dev/75) 🐍 `pytest`

- [Getting started](https://docs.pytest.org/en/stable/getting-started.html#our-first-test-run)
- [Testing good practices](https://docs.pytest.org/en/stable/goodpractices.html#test-discovery)
- [vs unittest](https://blog.j-labs.pl/2019/02/Pytest-why-its-more-popular-than-unittest)
- [Pytest for Data Scientist](https://towardsdatascience.com/pytest-for-data-scientists-2990319e55e6?source=social.tw)

#### ![50%](https://progress-bar.dev/50) ⚙️ Docker compose

- [Getting started](https://docs.docker.com/compose/gettingstarted/)
- [Bind mounts](https://docs.docker.com/storage/bind-mounts/)
- [Environment variables](https://docs.docker.com/compose/environment-variables/)
- [Reference](https://docs.docker.com/compose/reference/)

#### ![100%](https://progress-bar.dev/100) ☁️ `mkdocs` continuous deployment to S3

- [Deploy a website with AWS CDK](https://netosec.com/deploy-aws-s3-website-using-cdk-python/)
- [Add S3 bucket policy via AWS CDK](https://stackoverflow.com/questions/60310575/how-to-add-s3-bucketpolicy-with-aws-cdk)
- [Continuous deployment with AWS Codebuild](https://netosec.com/continuous-deployment-using-aws-codebuild-with-cdk-for-next-js/)
- [`buildspec.yml` reference](https://docs.aws.amazon.com/codebuild/latest/userguide/build-spec-ref.html#build-spec-ref-syntax)
- [Local MathJax with mkdocs](https://stackoverflow.com/questions/52624102/local-mathjax-with-mkdocs)

#### ![100%](https://progress-bar.dev/100) ⚙️ `hydra` config

##### Introduction

- [Intro to Hydra](https://hydra.cc/docs/intro)
- [Issue 407](https://github.com/facebookresearch/hydra/issues/407)
- [Issue 332](https://github.com/facebookresearch/hydra/issues/332)

##### vs `dynaconfig`

- [Comparison](https://python.libhunt.com/compare-dynaconf-vs-hydra)
- [Dynaconf](https://www.dynaconf.com/)

##### w/ `streamlit`

- [Compose API](https://hydra.cc/docs/experimental/compose_api)
- [Hydra issue](https://github.com/facebookresearch/hydra/issues/440)
- [Global hydra](https://github.com/facebookresearch/hydra/blob/master/hydra/core/global_hydra.py)

#### ![100%](https://progress-bar.dev/100) 🛠️ Windows Terminal

- [Cmder issue](https://github.com/cmderdev/cmder/issues/2121)
- [Generate guids for profiles](https://traviscolbert.net/blog/windows-terminal-generating-guids-for-your-profiles/)
- [Terminal splash](https://terminalsplash.com/)
- [It's time to install Windows Terminal](https://www.hanselman.com/blog/ItsTimeForYouToInstallWindowsTerminal.aspx)
- [Adding reaction GIFs](https://www.hanselman.com/blog/AddingReactionGifsForYourBuildSystemAndTheWindowsTerminal.aspx)
- [Terminus issue](https://github.com/Eugeny/terminus/issues/2131)
- [How to shorten `cmder` current path](https://stackoverflow.com/questions/44476521/display-current-directory-not-full-path)

#### ![90%](https://progress-bar.dev/90) ⚙️ WSL2 101

##### Setup

- [Install in Windows 10](https://docs.microsoft.com/it-it/windows/wsl/install-win10)
- [User support](https://docs.microsoft.com/it-it/windows/wsl/user-support)
- [Add user in WSL](https://www.tenforums.com/tutorials/128052-add-user-windows-subsystem-linux-wsl-distro-windows-10-a.html)
- [Getting started](https://code.visualstudio.com/docs/remote/wsl#_getting-started)

##### File systems

- [Create folder from windows then access from bash](https://superuser.com/questions/1127307/create-folder-from-windows-then-access-from-bash-in-windows-10-wsl)
- [Browsing a Unix path using Windows](https://superuser.com/questions/282963/browse-an-unc-path-using-windows-cmd-without-mapping-it-to-a-network-drive)

##### Packages

- [How to install make in Ubuntu](https://stackoverflow.com/questions/11934997/how-to-install-make-in-ubuntu)
- [User is not in the sudoers](https://www.tecmint.com/fix-user-is-not-in-the-sudoers-file-the-incident-will-be-reported-ubuntu/)
- [Switch users on Linux Ubuntu](https://net2.com/how-to-switch-users-on-linux-ubuntu/)

#### ![100%](https://progress-bar.dev/100) ⚙️ First Heroku deployment

- [Streamlit on Heroku](https://towardsdatascience.com/deploy-streamlit-on-heroku-9c87798d2088)
- [Deploying interactive Data Science apps](https://dev.to/hannahyan/getting-started-in-deploying-interactive-data-science-apps-with-streamlit-part-2-3ob)

### 08 / August

#### ![100%](https://progress-bar.dev/100) 🐍 `mkdocs` 101

- [Documenting a Python package](https://medium.com/@chrieke/documenting-a-python-package-with-code-reference-via-mkdocs-material-b4a45197f95b)
- [Official docs](https://www.mkdocs.org/)
- [`mkdocs-material`](https://squidfunk.github.io/mkdocs-material/)
- [`mkdocstrings`](https://pawamoy.github.io/mkdocstrings/)
- [YAML of mkdocs itself](https://github.com/pawamoy/mkdocstrings/blob/master/mkdocs.yml)
- [Linking documents](https://mkdocs.readthedocs.io/en/0.11.1/user-guide/writing-your-docs/#linking-documents)

#### ![60%](https://progress-bar.dev/60) ☁️ AWS CDK 101

- [Intro to AWS CDK](https://docs.aws.amazon.com/cdk/latest/guide/home.html)
- [AWS CDK EC2 key-pair](https://pypi.org/project/cdk-ec2-key-pair/)
- [Getting started](https://aws.amazon.com/it/blogs/developer/getting-started-with-the-aws-cloud-development-kit-and-python/)
- [CDK workshop](https://cdkworkshop.com/30-python.html)
- [Open CDK](https://github.com/kevinslin/open-cdk)
- [Official repo](https://github.com/aws/aws-cdk)
- [AWS CDK examples](https://github.com/aws-samples/aws-cdk-examples)

### 06 / June

#### ![100%](https://progress-bar.dev/100) ☁️ AWS EC2 101

- [AWS and Docker basics](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/docker-basics.html)
- [Streamlit fails for external IP not detected](https://discuss.streamlit.io/t/streamlit-fails-to-start-because-external-ip-not-detected/2378)
- [Override CMD when running a Docker image](https://serverfault.com/questions/594281/how-can-i-override-cmd-when-running-a-docker-image)

#### ![100%](https://progress-bar.dev/100) 🛠️ GNU Make

- [What and how about Makefile](https://opensource.com/article/18/8/what-how-makefile)
- [Make novice tutorial](https://swcarpentry.github.io/make-novice/02-makefiles/index.html)
- [Make official manual](https://www.gnu.org/software/make/manual/html_node/)
- [`pymake`](https://github.com/tqdm/py-make)
- [Make experiments](http://lifesum.github.io/posts/2016/01/14/make-experiments/)
- [Make vs shell scripts](https://stackoverflow.com/questions/3798562/why-use-make-over-a-shell-script)

### 03 / March

#### ![100%](https://progress-bar.dev/100) 🐍 Dataclass

- [Dataclasses](https://docs.python.org/3/library/dataclasses.html)
- [Python dataclasses](https://realpython.com/python-data-classes/)
- [Namedtuple vs Dataclass](https://towardsdatascience.com/understand-how-to-use-namedtuple-and-dataclass-in-python-e82e535c3691)
- [Dunderless Python](https://towardsdatascience.com/dunderless-python-7d7ab634450b)

#### ![100%](https://progress-bar.dev/100) ⚗️ Transportation problem

- [Find Santa with MILP](http://find-santa.eu/teaching/cms/milp.php)
- [PuLP transportation problem](https://coin-or.github.io/pulp/CaseStudies/a_transportation_problem.html)
- [Scip intro to transportation problem](https://scipbook.readthedocs.io/en/latest/intro.html#transportation-problem)
- [OR Tools min cost flow](https://developers.google.com/optimization/flow/mincostflow.html)
- [Transportation problem intro](https://www.geeksforgeeks.org/transportation-problem-set-1-introduction/)
- [Optimization for programmers](https://medium.com/@geekrodion/course-optimization-for-programmers-5316572ed69b)

#### ![100%](https://progress-bar.dev/100) 🛠️ tmux 101 and cmder

- [Tmux cheatsheet](https://tmuxcheatsheet.com/)
- [Cmder](https://cmder.net/)
- [Cmder issue](https://github.com/cmderdev/cmder/issues/2028)

### 02 / February

#### ![100%](https://progress-bar.dev/100) 🐍 `pandas-profiling` and `pyjanitor`

#### ![100%](https://progress-bar.dev/100) ⚙️ Docker 101

- [Streamlit in Docker](https://discuss.streamlit.io/t/how-to-use-streamlit-in-docker/1067/6)
- [Get started](https://docs.docker.com/get-started/)
- [How to see docker image contents](https://stackoverflow.com/questions/44769315/how-to-see-docker-image-contents)
- [Mount a directory inside a container](https://towardsdatascience.com/how-to-mount-a-directory-inside-a-docker-container-4cee379c298b)
- [Docker arg env variable](https://vsupalov.com/docker-arg-env-variable-guide/)
- [Remove images and containers](https://www.digitalocean.com/community/tutorials/how-to-remove-docker-images-containers-and-volumes#:~:text=Remove%20all%20images,docker%20images%20%2Da)

#### ![80%](https://progress-bar.dev/80) 🐍 Python virtual environment overview

- [`virtualenv` and Jupyter](https://towardsdatascience.com/create-virtual-environment-using-virtualenv-and-add-it-to-jupyter-notebook-6e1bf4e03415)
- [Python environment 101](https://towardsdatascience.com/python-environment-101-1d68bda3094d)
- [Guide to virtual envs](https://docs.python-guide.org/dev/virtualenvs/)
- [Jupyter and virtual envs](https://janakiev.com/blog/jupyter-virtual-envs/)
- [Devops for data science project](https://towardsdatascience.com/devops-for-data-science-making-your-python-project-reproducible-f55646e110fa)
- [Update requirements with `pur`](https://github.com/alanhamlett/pip-update-requirements)
- [Wrong jupyter kernels reference](https://github.com/jupyter/notebook/issues/2359#issuecomment-318061445)

#### ![100%](https://progress-bar.dev/100) ☁️ AWS SAM vs Chalice

- [Stage specific config](https://chalice.readthedocs.io/en/latest/topics/configfile.html#stage-specific-configuration)
- [Lambda specific config](https://chalice.readthedocs.io/en/latest/topics/configfile.html#lambda-specific-configuration)
- [Packaging](https://chalice.readthedocs.io/en/latest/topics/packaging.html)
- [Multifile](https://chalice.readthedocs.io/en/latest/topics/multifile.html)
- [Runtime version](https://chalice.readthedocs.io/en/latest/topics/pyversion.html#changing-python-runtime-versions)
- [Cloud formation](https://chalice.readthedocs.io/en/latest/topics/cfn.html)
- [API Gateway](https://chalice.readthedocs.io/en/latest/api.html)
- [Pure lambda functions](https://chalice.readthedocs.io/en/latest/topics/purelambda.html)
- [Events](https://chalice.readthedocs.io/en/latest/topics/events.html)
- [Chalice workshop](https://chalice-workshop.readthedocs.io/en/latest/media-query/index.html)

### 01 / January

#### ![100%](https://progress-bar.dev/100) ☁️ AWS 101 and AWS SAM

- [AWS SAM hello world](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-getting-started-hello-world.html)
- [AWS SAM serverless function](https://github.com/awslabs/serverless-application-model/blob/master/versions/2016-10-31.md#awsserverlessfunction)
- [Notification content structure](https://docs.aws.amazon.com/AmazonS3/latest/dev/notification-content-structure.html)
- [S3 client](https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3.html#S3.Client.get_object)
- [Custom AWS Lambda layer](https://stackoverflow.com/questions/58369170/build-custom-aws-lambda-layer-for-scikit-image/58408130#58408130)

#### ![100%](https://progress-bar.dev/100) ☁️ Publish `sphinx` docs as static website on AWS S3

- [Gist](https://gist.github.com/monkut/aa011550d596088ef577ad6d82722a20)

#### ![100%](https://progress-bar.dev/100) ☁️ AWS Codecommit

- [Setting up](https://docs.aws.amazon.com/codecommit/latest/userguide/setting-up-gc.html)
- [How to migrate local repo](https://docs.aws.amazon.com/codecommit/latest/userguide/how-to-migrate-repository-local.html)
- [How to connect to AWS Codecommit](https://docs.aws.amazon.com/codecommit/latest/userguide/how-to-connect.html)
- [Gitkraken HTTPS with AWS Codecommit](https://sharpi.sh/blog/web-development/workaround-for-gitkraken-ssh-and-https-not-workig-with-aws-codecommit/)

#### ![100%](https://progress-bar.dev/100) 🐍 Python package to install vs module to use with `__main__.py`

- [Minimal project packaging](https://python-packaging.readthedocs.io/en/latest/minimal.html)
- [Official docs](https://docs.python-guide.org/writing/structure/)
- [Dead simple Python project structure](https://dev.to/codemouse92/dead-simple-python-project-structure-and-imports-38c6)
- [How to build your first Python package](https://towardsdatascience.com/how-to-build-your-first-python-package-6a00b02635c9)
- [Python packaging tutorial](https://python-packaging-tutorial.readthedocs.io/en/latest/setup_py.html)
- [Recipe to organize data science projects](https://towardsdatascience.com/a-recipe-for-organising-data-science-projects-50a1cc539c69)
- [Guide to `setup.py`](https://godatadriven.com/blog/a-practical-guide-to-using-setup-py/)

#### ![100%](https://progress-bar.dev/100) 🐍 `pipreqs` for automate _requirements.txt_

#### ![100%](https://progress-bar.dev/100) 🛠️ Git 101 with gitkraken

- [Getting started with Git](https://towardsdatascience.com/version-control-101-getting-started-with-git-3b2c78d91184)

#### ![100%](https://progress-bar.dev/100) 🐍 `sphinx` documentation

## 2019 

### 12 / December

#### ![100%](https://progress-bar.dev/100) 📊 First `streamlit` app
#### ![100%](https://progress-bar.dev/100) 🐍 Visual Studio Code and JupyterLab

### 11 / November

#### ![100%](https://progress-bar.dev/100) 🐍 Python decorators

- [Intro to decorators](https://www.codementor.io/@sheena/introduction-to-decorators-du107vo5c)
- [A decorator to replace specific string in function](https://stackoverflow.com/questions/58515362/a-decorator-to-replace-specific-string-in-function-to-another-string)

### 08 / August

#### ![100%](https://progress-bar.dev/100) 🐍 Python class usage, inheritance and  `__init__` method

- [Intro to Python classes](http://introtopython.org/classes.html)